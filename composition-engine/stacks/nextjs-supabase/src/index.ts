...
export class NextJsSupabaseAdapter implements StackAdapter {
  ...
  public generate(plan: CompositionPlan): Record<string, string> {
    ...
    // 1. Database (Entities)
    // For each database entity, generate:
    // - DDL file: `supabase/migrations/[entityName]_table.sql`
    // - TypeScript type file: `types/database.ts` (with interface for each entity)
    ...
    // 2. API Artifacts
    // For each API endpoint, generate Next.js App Router API Route Handler
...
export async function ${method.toUpperCase()}(_request: Request) {
  // Placeholder implementation generated by Minecode NextJsSupabaseAdapter
...
`;
    ...
    // 4. Migrations
    // Copy feature migrations directly
    ...
    // 6. Permissions, Events, Extension Points (Project Glue)
