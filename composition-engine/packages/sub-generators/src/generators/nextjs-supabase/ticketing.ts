...
export class NextjsSupabaseTicketingSubGenerator implements SubGenerator {
  public readonly id = 'nextjs-supabase-ticketing';

  public generate(plan: CompositionPlan): Record<string, string> {
    const files: Record<string, string> = {};

    let hasTicketing = false;
    for (const apiDef of plan.api) {
      if (apiDef.path.includes('/api/tickets')) {
        hasTicketing = true;
      }
    }

    if (!hasTicketing) return files;
...
`;
    ...
    // UI: Ticket Inbox
    files['app/tickets/page.tsx'] = `import React, { useState, useEffect } from 'react';
...
export default function TicketInboxPage() {
  ...
  const fetchTickets = async () => {
    try {
      const res = await fetch('/api/tickets'); // Generic GET
